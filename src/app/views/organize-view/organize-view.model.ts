...
export class PropertyType {
  ...
  constructor(name: string, value: string, _group?: string) {
    this.name = name;
    this.value = value;
    this.group = '';
  }
...
export class AddMenuItemsManager extends BaseMenuItemsManager {
  changeItemsAccessibility(_actionSource: number) {
                ...
                if (
                  ...
                  for (const subItem of menuItem.subItems) {
                    if (
                      ...
                      subItem.enabled = this.supportedExtensionItem();
                    }
                  }
                } else {
                  menuItem.visible = false;
            ...
            case ActionMenuItemsManager.SOURCE_FILE_BROWSER:
              if (this.selectedItem.length === 0) {
                ...
                if (
                  ...
                  menuItem.visible = this.supportedExtensionItem();
                  menuItem.enabled = this.supportedExtensionItem();
                  for (const subItem of menuItem.subItems) {
                    if (
                      (subItem.name === ContextMenu.EXPORT_TO_CSV && this.selectedItemExtension() !== 'csv') ||
