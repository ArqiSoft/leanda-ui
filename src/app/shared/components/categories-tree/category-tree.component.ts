...
export class CategoryTreeComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private entityService: EntityCountsService,
  ...
  ngOnInit(): void {
    ...
    this.service.tree$
      .pipe(takeUntil(this.destroy$))
      .subscribe(tree =>
        tree && tree.nodes ? (this.dataSource.data = tree.nodes) : null
      );
    ...
    this.nodesApi
      ...
      .subscribe(
        (result: { data: []; page: { totalCount: number } }) =>
          (this.entityService.entities.find(k => k.key === "all").count =
            result.page.totalCount || 0)
      );

    this.entitiyFilter = this.entityService.getCounter(
      EEntityFilter.ALL
    );
    this.currentFilter = this.entityService.activeFilter;
  ...
  displayAllFiles(): void {
    ...
    this.entityService.activeFilter = this.currentFilter =
      EEntityFilter.ALL;
